...
public class Evaluation {
	 ...
	 * yield the exact same predictions for all tasks. *
	 * 
	 */

	printHeader1("II) Operator Groups: Identical Predictions");
	redundantOperators = new HashSet<>();
	Set<List<DataOperator>> operatorPredictionGroups = groupOperatorsByPrediction();
	...
	printHeader2("The following " + operatorPredictionGroups.size()
		+ " groups of operators give the exact same predictions for all the given tasks. -> Keep the first operator as representative for a group and exclude "
	...
	printHeader1("IV) Answers Deviating from Classical Logic");
	printHeader1("IV-1) Overall Frequency of Logically Inconsistent Answers");

	Map<Pair<UniqueTask, Formula<PropositionalSignature<Character>>>, Integer> frequentErrors = getMostFrequentErrors();

	printHeader1("IV-2) Participants' Answers");
	for (Participant participant : participants) {

	    printHeader2("Logically inconsistent answers given by participant: " + participant);

	    System.out.println("Number of answers deviating from classical logic: " + participant.wrongAnswers.size()
		    + " of " + participant.pRecords.size());
	    System.out.println();
	    for (DataRecord wrongAnswer : participant.wrongAnswers) {
		System.out.println(wrongAnswer);
	    }

	}

	printHeader1("IV-3) Individual Response Patterns");
	int counter = 1;
	List<Pair<UniqueTask, Formula<PropositionalSignature<Character>>>> errorList = new ArrayList<>(
		frequentErrors.keySet());
	for (Pair<UniqueTask, Formula<PropositionalSignature<Character>>> error : errorList) {
	    System.out.println("Deviation " + counter + ": " + error);
	    counter++;
	}

	// Error vectors and clusterer for Java ML
//	Dataset clusteringData = new DefaultDataset();
	Dataset clusteringData = buildErrorVectors(counter, errorList);
//	buildErrorVectors(counter, errorList, clusteringData);
	printHeader1("IV-4) Reasoner Types");
	printHeader2(
		"The following teypes are based on the individual response patterns that deviate from classical logic.");

	System.out.println();
	System.out.println();

	Clusterer c = new DensityBasedSpatialClustering(0.33, 2);
	Dataset[] clusters = c.cluster(clusteringData);

	for (Dataset entry : clusters) {
	    System.out.print("Cluster: ");
//	    System.out.print("| ");
	    for (Instance elem : entry)
		System.out.print(elem.classValue() + ", ");
	    System.out.println();
	}
//	
    }
    ...
    /**
     * This function constructs and outputs a vector that represents the errors a
     * participant made. It also adds the vectors to a DataSet for the clusterer of
     * Java ML
     * 
     * @param counter   Number of the error
     * @param errorList The List of frequent errors in this data set.
     * @return DataSet with all error vectors.
     */
    private static Dataset buildErrorVectors(int counter,
	    List<Pair<UniqueTask, Formula<PropositionalSignature<Character>>>> errorList) {
	Dataset clusteringData = new DefaultDataset();
	for (Participant participant : participants) {
	    double[] tempVector = new double[errorList.size()];
	    for (DataRecord record : participant.wrongAnswers) {
		for (int i = 0; i < errorList.size(); i++) {

		    if (errorList.get(i).getFirst().premises.equals(record.task)) {
			if (!(errorList.get(i).getSecond() == null)
				&& (errorList.get(i).getSecond().equals(record.response))) {
			    tempVector[i] += 1;
	    ...
	    for (double entry : tempVector) {
		System.out.print(entry + ", ");
	    }
	    System.out.println();
	    Instance vector = new DenseInstance(tempVector, participant.id);
	    clusteringData.add(vector);
    ...
    /**
     * This function groups the operators by their predictions for the task. Operators are assigned a
     * group only if the criterion is equal for every task. If the prediction differs for one
     * task, the two operators are not grouped.
     * 
     * @return A set of operator lists grouped by the given criterion.
     */
    private static Set<List<DataOperator>> groupOperatorsByPrediction() {
...
//	double accuracyOther = (double) countOtherTrue / countOther;
    ...
    /**
     * This function computes the most frequent errors the participants made with a
     * support of 1/5.
     * 
     * @return A Map that assigns the frequency of its occurrence to the most
     *         frequent false task/answer combinations.
     */
    private static Map<Pair<UniqueTask, Formula<PropositionalSignature<Character>>>, Integer> getMostFrequentErrors() {
	Map<Pair<UniqueTask, Formula<PropositionalSignature<Character>>>, Integer> frequentErrors = new HashMap<>();

	int absSupport = participants.size() / 5;

	// Counts occurence of a specific answer for a task.
	for (UniqueTask task : listOfTasks) {
	    for (Map.Entry<Formula<PropositionalSignature<Character>>, Integer> freq : task.wrongAnswerFrequencies
		    .entrySet()) {

		System.out.println(task + " Answer: " + freq.getKey() + " | Frequency = " + freq.getValue());

		if (freq.getValue() > absSupport) {
		    frequentErrors.put(
			    new Pair<UniqueTask, Formula<PropositionalSignature<Character>>>(task, freq.getKey()),
			    freq.getValue());
		}
	...
	System.out.printf("%-6s %-30s%n", "II)", "Operator Groups: Identical Predictions");
	...
	System.out.printf("%-6s %-30s%n", "IV)", "Answers Deviating from Classical Logic");
	System.out.printf("%-6s %-30s%n", "IV-1)", "Overall Frequency of Logically Inconsistent Answers");
	System.out.printf("%-6s %-30s%n", "IV-2)", "Participants' Answers");
	System.out.printf("%-6s %-30s%n", "IV-3)", "Individual Response Patterns");
	System.out.printf("%-6s %-30s%n", "IV-4)", "Reasoner Types");
    }
